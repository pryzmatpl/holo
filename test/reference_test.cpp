...
TEST(HoloeyeMovAvgTest, ConstantInputReturnsZero) {
    ...
    eit_hologram holo;
    Mat output = holo.holoeye_movavg(input);
...
TEST(HoloeyeRefWavefrontFlatPhaseTest, AppliesConstantPhaseShift) {
    ...
    eit_hologram holo;
    holo.holoeye_ref_wavefront_flat_phase(input, angle);
...
TEST(HoloeyeRefWavefrontPhase2Test, ReturnsPhaseImageInRange) {
    ...
    eit_hologram holo;
    Mat output = holo.holoeye_ref_wavefront_phase2(input);
...
TEST(HoloeyeChirpTest, OutputSizeAndType) {
    ...
    Mat input(3, 3, CV_32FC2, Scalar(1.0f, 0.0f));
    eit_hologram holo;
    Mat output = holo.holoeye_chirp(input, 1.0f);
...
TEST(HoloeyeReferenceTest, FlatReference) {
    ...
    eit_hologram holo;
    Mat output = holo.holoeye_reference(input, "FLAT");
...
TEST(HoloeyeReferenceTest, UnsupportedTypeDoesNotCrash) {
    ...
    eit_hologram holo;
    Mat output = holo.holoeye_reference(input, "UNKNOWN");
    EXPECT_EQ(output.size(), input.size());
}
...
TEST(HoloeyeRefWavefrontSquareApertureTest, OutputSizeAndType) {
    ...
    eit_hologram holo;
    Mat output = holo.holoeye_ref_wavefront_squareaperture(input, 1.0f);
    EXPECT_EQ(output.size(), input.size());
    EXPECT_EQ(output.type(), input.type());
}
...
TEST(HoloeyeRefWavefrontFocalTest, OutputSizeAndType) {
    Mat input(4, 4, CV_32FC2, Scalar(1.0f, 0.0f));
    eit_hologram holo;
    holo.holoeye_ref_wavefront_focal(input);
...
TEST(HoloeyeRefWavefrontSommerfeldTest, OutputSizeAndType) {
    Mat input(4, 4, CV_32FC2, Scalar(1.0f, 0.0f));
    eit_hologram holo;
    Mat output = holo.holoeye_ref_wavefront_sommerfeld(input, 1.0f);
    EXPECT_EQ(output.size(), input.size());
    EXPECT_EQ(output.type(), input.type());
}
...
TEST(HoloeyeRefWavefrontPhaseTest, OutputSizeAndType) {
    Mat input(4, 4, CV_32FC2, Scalar(1.0f, 0.0f));
    eit_hologram holo;
    Mat output = holo.holoeye_ref_wavefront_phase(input);
    EXPECT_EQ(output.size(), input.size());
